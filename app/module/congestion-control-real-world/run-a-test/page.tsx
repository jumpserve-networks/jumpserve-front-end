...
export default async function RealWorldPage() {
  ...
  return <main className="mx-auto max-w-6xl px-4 py-8"><h1 className="text-2xl font-semibold">Congestion Control Real World Tests</h1>
    ...
    {user ? <RealWorldTests /> : <div className="mt-6 space-y-4"><SignInToRun nextPath={`${REAL_WORLD_MODULE_PATH}/run-a-test`} /><Link href={`${REAL_WORLD_MODULE_PATH}/real-world-reports`} className="text-sm underline underline-offset-4">Browse test results</Link></div>}
  </main>;
}
