  ...
  return <div className="mt-6 space-y-6">
    ...
    {!job ? <p role="status" className="text-sm text-muted-foreground">Loading test…</p> : <>
      ...
      {(job.results || isRealWorldTerminal(job.status)) && <Card><CardHeader><CardTitle>Measurements</CardTitle></CardHeader><CardContent className="space-y-4">
        ...
        <div className="flex flex-wrap gap-3"><Button size="sm" nativeButton={false} render={<Link href={`${REAL_WORLD_MODULE_PATH}/real-world-reports/${jobId}`} />}>View test results</Button><Button variant="outline" size="sm" disabled={busy} onClick={() => void download()}>Get raw report links</Button></div>
