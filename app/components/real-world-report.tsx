...
export function RealWorldReport({ jobId }: { jobId: string }) {
  ...
  return <article className="real-world-report mt-6 space-y-6">
    ...
    <Card><CardHeader><CardTitle>Provenance and exports</CardTitle></CardHeader><CardContent className="space-y-3 text-sm">
      ...
      <p className="text-muted-foreground">CSV exports preserve units and clock definitions. Report JSON includes configurations, eligibility checks, sample counts, and source checksums. Raw links expire after five minutes; anyone holding a link can use it until expiry.</p>
      <Table><TableHeader><TableRow><TableHead>Source</TableHead><TableHead>SHA-256</TableHead><TableHead>S3 version</TableHead></TableRow></TableHeader><TableBody>{report.sources.map(source => <TableRow key={source.name}><TableCell>{source.name}</TableCell><TableCell className="max-w-xs break-all whitespace-normal font-mono text-xs">{source.sha256}</TableCell><TableCell className="max-w-xs break-all whitespace-normal font-mono text-xs">{source.version_id ?? "Unavailable"}</TableCell></TableRow>)}</TableBody></Table>
